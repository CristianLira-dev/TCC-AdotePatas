  ...
  const init = () => {
    ...
    const sendFile = async (input, event) => {
      ...
      try {
        ...
        const time = el.querySelector('.message-timestamp');
        if (time) {
          time.childNodes.forEach((node) => { if (node.nodeType === Node.TEXT_NODE) node.textContent = ''; });
          time.prepend(document.createTextNode((result.data_formatada || 'Enviado') + ' '));
        }
        ...
        el.classList.remove('is-pending');
        const time = el.querySelector('.message-timestamp');
        if (time) time.textContent = 'Falha ao enviar';
